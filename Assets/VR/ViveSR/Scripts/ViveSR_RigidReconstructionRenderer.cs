...
namespace Vive.Plugin.SR
{
    public class ViveSR_RigidReconstructionRenderer : MonoBehaviour
    {
        ...
        public bool UpdateRuntimeParameter()
        {
            ...
            // update adaptive param
            if (LiveMeshDisplayMode == ReconstructionDisplayMode.ADAPTIVE_MESH)
            {
                ret = ViveSR_Framework.SetParameterFloat(ViveSR_Framework.MODULE_ID_RIGID_RECONSTRUCTION, (int)ReconstructionParam.ADAPTIVE_MAX_GRID, ViveSR_RigidReconstruction.AdaptiveMaxGridSize * 0.01f);   // cm to m
                result = result && (ret == (int)Error.WORK);
                ret = ViveSR_Framework.SetParameterFloat(ViveSR_Framework.MODULE_ID_RIGID_RECONSTRUCTION, (int)ReconstructionParam.ADAPTIVE_MIN_GRID, ViveSR_RigidReconstruction.AdaptiveMinGridSize * 0.01f);
                result = result && (ret == (int)Error.WORK);
                ret = ViveSR_Framework.SetParameterFloat(ViveSR_Framework.MODULE_ID_RIGID_RECONSTRUCTION, (int)ReconstructionParam.ADAPTIVE_ERROR_THRES, ViveSR_RigidReconstruction.AdaptiveErrorThres);
                result = result && (ret == (int)Error.WORK);
            }                 
            return result;
        }
