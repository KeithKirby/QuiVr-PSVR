...
Shader "DLNK Shaders/Complex/WaterFoamTes" {
    Properties {
        ...
        _FMaskMult ("FMask Mult", Float ) = 1
        _FoamColor ("Foam Color", Color) = (1,1,1,1)
        _FoamAlbedo ("Foam Albedo", 2D) = "white" {}
        _FoamPower ("Foam Power", Float ) = 1
        _FoamGloss ("Foam Gloss", 2D) = "white" {}
        _FoamGlossMult ("Foam Gloss Mult", Float ) = 1
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _FoamAlbedo; uniform float4 _FoamAlbedo_ST;
            uniform sampler2D _FoamGloss; uniform float4 _FoamGloss_ST;
            uniform float _FoamGlossMult;
            uniform sampler2D _FoamMask; uniform float4 _FoamMask_ST;
            uniform float _FMaskMult;
            ...
            VertexOutput vert (VertexInput v) {
                ...
                void displacement (inout VertexInput v){
                    ...
                    float4 _Depth1 = tex2Dlod(_ParallaxMap,float4(TRANSFORM_TEX(node_4749, _ParallaxMap),0.0,0));
                    ...
                    float4 _node_5929 = tex2Dlod(_ParallaxMap,float4(TRANSFORM_TEX(node_8067, _ParallaxMap),0.0,0));
...
///////// Gloss:
                ...
                float4 _FoamGloss_var = tex2D(_FoamGloss,TRANSFORM_TEX(i.uv0, _FoamGloss));
                float4 _node_500 = tex2D(_FoamMask,TRANSFORM_TEX(node_4749, _FoamMask));
                float4 _node_5748 = tex2D(_FoamMask,TRANSFORM_TEX(node_8067, _FoamMask));
                float node_2643 = saturate((saturate(( _node_5748.r > 0.5 ? (1.0-(1.0-2.0*(_node_5748.r-0.5))*(1.0-_node_500.r)) : (2.0*_node_5748.r*_node_500.r) ))*_FMaskMult));
                float gloss = lerp(node_5886,(_FoamGloss_var.r*_FoamGlossMult),node_2643);
...
////// Specular:
                ...
                float4 _node_9310 = tex2D(_FoamAlbedo,TRANSFORM_TEX(node_4749, _FoamAlbedo));
                float4 _node_8595 = tex2D(_FoamAlbedo,TRANSFORM_TEX(node_8067, _FoamAlbedo));
                float3 diffuseColor = lerp((lerp(_MainTexture_var.rgb,_WaterTextureB_var.rgb,_UseMainTexAlpha_var)*_MainColor.rgb),(saturate(( _node_8595.rgb > 0.5 ? (1.0-(1.0-2.0*(_node_8595.rgb-0.5))*(1.0-_node_9310.rgb)) : (2.0*_node_8595.rgb*_node_9310.rgb) ))*_FoamPower*_FoamColor.rgb),node_2643); // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 _node_5748 = tex2D(_FoamMask,TRANSFORM_TEX(node_8067, _FoamMask));
                float node_2643 = saturate((saturate(( _node_5748.r > 0.5 ? (1.0-(1.0-2.0*(_node_5748.r-0.5))*(1.0-_node_500.r)) : (2.0*_node_5748.r*_node_500.r) ))*_FMaskMult));
                float3 diffColor = lerp((lerp(_MainTexture_var.rgb,_WaterTextureB_var.rgb,_UseMainTexAlpha_var)*_MainColor.rgb),(saturate(( _node_8595.rgb > 0.5 ? (1.0-(1.0-2.0*(_node_8595.rgb-0.5))*(1.0-_node_9310.rgb)) : (2.0*_node_8595.rgb*_node_9310.rgb) ))*_FoamPower*_FoamColor.rgb),node_2643);
                float specularMonochrome;
                float3 specColor;
                ...
                float node_5886 = (lerp(_WaterAGloss_var.r,(_WaterBGloss_var.r*_SecondaryGloss),_UseMainTexAlpha_var)*_Gloss);
                float4 _FoamGloss_var = tex2D(_FoamGloss,TRANSFORM_TEX(i.uv0, _FoamGloss));
                float roughness = 1.0 - lerp(node_5886,(_FoamGloss_var.r*_FoamGlossMult),node_2643);
    ...
    FallBack "DLNK Shaders/Advanced/WaterFoam"
