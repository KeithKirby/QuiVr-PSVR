...
public class PhotonVoiceRecorder : Photon.MonoBehaviour
{
    ...
    private void Update()
    {
        if(_wasTransmitting != IsTransmitting)
        {
            _wasTransmitting = IsTransmitting;
            if (IsTransmitting)
    ...
    void Start()
    {
        ...
        if (photonView.isMine)
        {
            ...
            if (this.voice != Voice.LocalVoice.Dummy)
            {
                ...
                Debug.LogWarningFormat("PUNVoice: AutoTransmit set to {0}", PhotonVoiceSettings.Instance.AutoTransmit);
            } 
