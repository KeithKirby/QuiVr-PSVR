...
namespace BeautifulDissolves {
	public class Dissolve : MonoBehaviour {
        ...
        public void ResetDissolve()
        {
            ...
            {
                DissolveHelper.SetDissolveAmount(v, 0);
            }
        }

        public void SetDissolveAmount(float Amt)
        {
            if (m_Materials == null)
                return;
            foreach(var v in m_Materials)
            {
                DissolveHelper.SetDissolveAmount(v, Amt / 2f); //Max is 0.5f
            }
        }
		...
		{
			if ((atomic && m_Dissolving) || !gameObject.activeInHierarchy) {
				return;
			}
			...
			if (m_Materials != null && m_Materials.Length > 0) {
				if (!(oneTime && m_Dissolving)) {
                    ...
                    if (speed < 0)
                        reverse = true;
					StartCoroutine(YieldDissolve(m_CurrentCoroutine, reverse));
				}
			}
		}
		...
		IEnumerator YieldDissolve(IEnumerator coroutine, bool reverse=false)
		{
			...
			m_Dissolving = false;
            if (DoActivation && !reverse)
                UnityMainThreadDispatcher.Instance().Enqueue(delegate { gameObject.SetActive(false); });               
		}
