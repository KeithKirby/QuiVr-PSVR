...
namespace Tobii.VR
{
    ...
    [RequireComponent(typeof(TobiiVR_CalibrationViewModel))]
    public class TobiiVR_CalibrationView : MonoBehaviour
    {
        // The calibration dots should be in front of 
        // the user and currently requires minimum 5
        private readonly Vector3[] _calibrationPoints = { new Vector3(0f, 0f, 1f), new Vector3(0.2f, 0f, 1f), new Vector3(-0.2f, 0f, 1f), new Vector3(0f, 0.2f, 1f), new Vector3(0f, -0.2f, 1f) };

        public GameObject TargetPrefab;
        public KeyCode CalibrationKey = KeyCode.F1;

        private GameObject _instance;
        private Vector3 _originalScale;
        private TobiiVR_CalibrationViewModel _calibrationViewModel;

        void Awake()
        {
            TobiiVR_Host.Instance.Init();
            _calibrationViewModel = GetComponent<TobiiVR_CalibrationViewModel>();
        }

        void Update()
        {
            if (Input.GetKeyUp(CalibrationKey))
            {
                InitiateCalibration();
            }
        }
        ...
        {
            if (TobiiVR_Host.Instance.IsCalibrating)
            {
                TobiiVR_Logging.Log("Cannot start multiple calibrations at the same time.");
                return false;
            }

            if (_instance != null)
            {
                TobiiVR_Logging.Log("Cannot start calibration as cleaning up is still under going.");
                return false;
            }

            if (TargetPrefab == null)
            {
                TobiiVR_Logging.Log("No calibration target assigned, cannot start calibration.");
                return false;
            }
            ...
            _instance.transform.parent = TobiiVR_Host.Instance.LocalToWorldTransform;
            _instance.transform.localScale = _originalScale;

            if (_calibrationViewModel.TryLaunchCalibration(_calibrationPoints))
            {
                TobiiVR_Logging.Log("Launching calibration.");
                _calibrationViewModel.WaitingOnIntroAnimation += StartIntroAnimation;
                _calibrationViewModel.WaitingOnOutroAnimation += StartOutroAnimation;
                _calibrationViewModel.OnCalibrationCompleted += OnCalibrationCompleted;
            }
            else
            {
                TobiiVR_Logging.Log("Failed to launch calibration.");
                _instance.transform.localPosition = _calibrationPoints[0];
                OnCalibrationCompleted(this, new CalibrationCompletedArgs(CalibrationResults.Failed));
                return false;
            }
        ...
        private void StartIntroAnimation(object sender, IntroAnimationArgs e)
        {
            ...
            StartCoroutine(e.CalibrationIndex == 0
                ? AppearAnimation(_calibrationPoints[e.CalibrationIndex])
                : TransistionAnimation(_calibrationPoints[e.CalibrationIndex]));
        }
        ...
        private IEnumerator AppearAnimation(Vector3 finalPosition)
        {
            ...
            _calibrationViewModel.HasCompletedAnimation = true;
        }
        ...
        private IEnumerator TransistionAnimation(Vector3 finalPosition)
        {
            ...
            while (progress < 1f)
            {
                ...
                var easing = Easings.QuadraticEaseOut(progress);
                _instance.transform.localPosition = startPosition + v * easing;
                _instance.transform.localScale = Vector3.Lerp(Vector3.zero, _originalScale, easing);
        ...
        private IEnumerator CalibratedPointAnimation()
        {
            ...
            while (progress < 1f)
            {
                ...
                _instance.transform.localScale = startScale * (1 - Easings.QuadraticEaseOut(progress));
                yield return null;
            }
        ...
        private IEnumerator DisplayCalibrationResult(CalibrationCompletedArgs e)
        {
            switch (e.Result)
            {
                case CalibrationResults.Successfully:
                    _instance.GetComponent<Renderer>().material.color = Color.green;
                    TobiiVR_Logging.Log("Successfully calibrated the user.");
                    break;
                case CalibrationResults.Failed:
                    _instance.GetComponent<Renderer>().material.color = Color.red;
                    TobiiVR_Logging.LogWarning("Failed to calibrate the user.");
                    break;
                default:
            ...
            while (progress < 1f)
            {
                ...
                _instance.transform.localScale = Vector3.Lerp(Vector3.zero, _originalScale, Easings.QuadraticEaseOut(progress));
