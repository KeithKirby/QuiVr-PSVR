...
public class SteamVR_Render : MonoBehaviour
{
	...
	private void OnInputFocus(bool hasFocus)
	{
		...
		{
			if (pauseGameWhenDashboardIsVisible && !PhotonNetwork.inRoom)
			{
				timeScale = Time.timeScale; 
				Time.timeScale = 0.01f; //JFS
            }
...
#if UNITY_2017_1_OR_NEWER
	void OnBeforeRender() {
        if (!Settings.GetBool("FilterHands") && Time.frameCount != lastFrameCount) //JFS
        {
    ...
    void Update()
	{
        ...
        //JFS
        if (Settings.GetBool("FilterHands") && Time.frameCount != lastFrameCount)
        {
            lastFrameCount = Time.frameCount;
            UpdatePoses();
        }
