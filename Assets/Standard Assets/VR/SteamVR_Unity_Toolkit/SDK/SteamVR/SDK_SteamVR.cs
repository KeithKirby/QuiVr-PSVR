...
#if UNITY_PS4
        // Don't init steam on ps4
#else
        [RuntimeInitializeOnLoadMethod]
        private void Initialise()
        {
            var executingAssembly = Assembly.GetExecutingAssembly();
            var eventClass = executingAssembly.GetType("SteamVR_Utils").GetNestedType("Event");
            var targetMethod = typeof(SDK_SteamVR).GetMethod("OnTrackedDeviceRoleChanged", BindingFlags.NonPublic | BindingFlags.Instance);

            if (eventClass == null)
            {
                //SteamVR plugin >= 1.2.0
                eventClass = executingAssembly.GetType("SteamVR_Events");
                var systemMethod = eventClass.GetMethod("System", BindingFlags.Public | BindingFlags.Static);
                var steamVREventInstance = systemMethod.Invoke(null, new object[] { "TrackedDeviceRoleChanged" });
                var listenMethod = steamVREventInstance.GetType().GetMethod("Listen", BindingFlags.Public | BindingFlags.Instance);
                var listenMethodParameterType = listenMethod.GetParameters()[0].ParameterType;
            ...
            {
                //SteamVR plugin <= 1.2.0
                var listenMethod = eventClass.GetMethod("Listen", BindingFlags.Public | BindingFlags.Static);
                var listenMethodParameterType = listenMethod.GetParameters()[0].ParameterType;
