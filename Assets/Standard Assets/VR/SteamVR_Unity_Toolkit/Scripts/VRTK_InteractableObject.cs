...
namespace VRTK
{
    ...
    public class VRTK_InteractableObject : MonoBehaviour
    {
        ...
        [Tooltip("Allow grip override if using Knuckles Controllers")]
        public bool FingerGripCanUse;
        ...
        protected virtual void Awake()
        {
            ...
            autoRigidbody = false;
            if (!AttachIsStaticObject() && isGrabbable)
            {
        ...
        protected virtual void Update()
        {
            ...
            if(FingerGripCanUse && KnucklesHandControl.UsingFingers)
            {
                GameObject touching = GetTouching();
                if (touching != null)
                {
                    VRTK_InteractGrab grab = touching.GetComponent<VRTK_InteractGrab>();
                    if(grab.GetGrabbedObject() == null)
                    {
                        KnucklesHandControl pos = touching.GetComponent<KnucklesHandControl>();
                        if(pos != null)
                        {
                            ...
                            if (pose.AvgVal() < 0.5f)
                            {
                                ...
                                e.interactingObject = touching;
                                InteractableObjectUsed.Invoke(touching, e);
                            }
                        }
