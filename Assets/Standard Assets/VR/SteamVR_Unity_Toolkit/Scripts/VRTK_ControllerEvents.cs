...
#region Variables
        ...
        public MotionControllerInput.Handed Hand;
        [HideInInspector]
        public uint controllerIndex;
        ...
        {
            var nullBool = false;
            return SetButtonEvent(ref nullBool);
        }

        private ControllerInteractionEventArgs SetButtonEvent(ref bool buttonBool, bool value=false, float buttonPressure=0f)
        {
            buttonBool = value;
        ...
        {
#if UNITY_WSA
            if (Hand == MotionControllerInput.Handed.Left)
                controllerIndex = 1;
            else
                controllerIndex = 0;
            return;
        ...
        private void Update()
        {
...
#if UNITY_WSA
            InteractionManager.SourceEventArgs ctr = MotionControllerInput.Controller(controllerIndex);
            InteractionSourceState src = ctr.state;
            Vector2 currentTriggerAxis = new Vector2((float)src.selectPressedValue, 0);
            TriggerValue = (float)src.selectPressedValue;
            Vector2 currentTouchpadAxis = new Vector2((float)src.controllerProperties.touchpadX, (float)src.controllerProperties.touchpadY);

            //Trigger Pressed
            if (src.selectPressed && !selectPressed)
            {
                selectPressed = true;
            ...
            else if (!src.selectPressed && selectPressed)
            {
                selectPressed = false;
                OnTriggerReleased(SetButtonEvent(ref triggerPressed, false, 0f));
                EmitAlias(ButtonAlias.Trigger_Press, false, 0f, ref triggerPressed);
            ...
            //ApplicationMenu
            if (src.menuPressed && !appmenuPressed)
            {
                Debug.Log("[WSA] Menu Pressed");
                appmenuPressed = true;
            ...
            else if (!src.menuPressed && appmenuPressed)
            {
            ...
            //Touchpad Pressed
            if (src.controllerProperties.touchpadPressed && !wsatppressed)
            {
                wsatppressed = true;
            ...
            else if (!src.controllerProperties.touchpadPressed && wsatppressed)
            {
                wsatppressed = false;
                OnTouchpadReleased(SetButtonEvent(ref touchpadPressed, false, 0f));
                EmitAlias(ButtonAlias.Touchpad_Press, false, 0f, ref touchpadPressed);
            ...
            touchpadAxis = new Vector2(currentTouchpadAxis.x, currentTouchpadAxis.y);
            triggerAxis = new Vector2(currentTriggerAxis.x, currentTriggerAxis.y);
            TriggerValue = currentTriggerAxis.x;
            hairTriggerDelta = VRTK_SDK_Bridge.GetTriggerHairlineDeltaOnIndex(controllerIndex);
#endif
        }

#if UNITY_PS4
        bool _hairTrigger = false;
        static float s_triggerThreshold = 0.1f;
        static int s_triggerMax = 255;

        void UpdatePS4Input()
        {
            int handId = Hand == MotionControllerInput.Handed.Left ? 1 : 0;
            var cs = PS4InputEx.GetMove(handId);
            // Read trigger values
#if UNITY_EDITOR
            float triggerX = 0;
#else
            float triggerX = (float)PS4Input.MoveGetAnalogButton(0, handId) / s_triggerMax;            
#endif
            var grabRight = PS4InputEx.GrabRight();
            triggerX = Mathf.Max(grabRight, triggerX);

            Vector2 currentTriggerAxis = new Vector2(triggerX, 0);
            ...
            if (cs.Trigger.Down)
            {
                OnTriggerPressed(SetButtonEvent(ref triggerPressed, true, 1));
                EmitAlias(ButtonAlias.Trigger_Press, true, 1, ref triggerPressed);
            }
            if (cs.Trigger.Up)
            {
                OnTriggerReleased(SetButtonEvent(ref triggerPressed, false, 0f));
            ...
            bool hairTrigger = currentTriggerAxis.x > s_triggerThreshold;
            if(_hairTrigger != hairTrigger)
            {
                if (hairTrigger)
                {
                    OnTriggerHairlineStart(SetButtonEvent(ref triggerHairlinePressed, true, currentTriggerAxis.x));
        ...
        {
#if UNITY_WSA
            controllerVelocity = MotionControllerInput.GetControllerVel(controllerIndex);
            controllerAngularVelocity = MotionControllerInput.GetControllerAngularVel(controllerIndex);
            return;
#endif
