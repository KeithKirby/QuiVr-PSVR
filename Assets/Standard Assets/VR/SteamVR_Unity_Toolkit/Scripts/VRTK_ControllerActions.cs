...
namespace VRTK
{
    ...
    public class VRTK_ControllerActions : MonoBehaviour
    {
...
#if UNITY_PS4
            PS4InputEx.HapticPulse(controllerIndex, (float)hapticPulseStrength / (float)maxHapticVibration);
#elif UNITY_WSA
            MotionControllerInput.TriggerHapticPulse(controllerIndex, (float)strength / (float)maxHapticVibration);
#else
        ...
        public void TriggerHapticPulse(AudioClip clip, float mult=1)
        {
            //StopAllCoroutines();
            StartCoroutine(AudioClipHapticsRoutine(clip, mult));
        }

        public void TriggerHapticPulse(AnimationCurve curve, float dur = 0.5f, float mult = 1f)
        {
            //StopAllCoroutines();
            StartCoroutine(CurveHapticsRoutine(curve, dur, mult));
        }
        ...
        private IEnumerator HapticPulse(float duration, ushort hapticPulseStrength, float pulseInterval)
        {
            ...
            while (duration > 0)
            {
                ...
                PS4InputEx.HapticPulse(controllerIndex, (float)hapticPulseStrength / (float)maxHapticVibration);
#elif UNITY_WSA
                MotionControllerInput.TriggerHapticPulse(controllerIndex, (float)hapticPulseStrength / (float)maxHapticVibration);
#else
                VRTK_SDK_Bridge.HapticPulseOnIndex(controllerIndex, hapticPulseStrength);
#endif
        ...
        private IEnumerator CurveHapticsRoutine(AnimationCurve curve, float dur, float mult)
        {
            float t = 0;
            if (dur <= 0)
                dur = 0.05f;
            while(t<1)
            {
                ushort hapticPulseStrength = (ushort)(ushort.MaxValue * curve.Evaluate(t)*mult);
#if UNITY_PS4
                PS4InputEx.HapticPulse(controllerIndex, (float)hapticPulseStrength / (float)maxHapticVibration);
#elif UNITY_WSA
                MotionControllerInput.TriggerHapticPulse(controllerIndex, (float)hapticPulseStrength / (float)maxHapticVibration);
        ...
        private IEnumerator AudioClipHapticsRoutine(AudioClip clip, float mult)
        {
            float[] audioData = new float[8192];
            int sampleOffset = -8192;
            ...
            {
                float lerpVal = (Time.unscaledTime - startTime) / length;
                int sampleIndex = (int)(sampleRate * lerpVal);
                if (sampleIndex >= sampleOffset + 8192)
                {
                ...
                ushort hapticPulseStrength = (ushort)(ushort.MaxValue * currentSample * mult);
#if UNITY_PS4
                PS4InputEx.HapticPulse(controllerIndex, (float)hapticPulseStrength / (float)maxHapticVibration);
#elif UNITY_WSA
                MotionControllerInput.TriggerHapticPulse(controllerIndex, (float)hapticPulseStrength / (float)maxHapticVibration);
