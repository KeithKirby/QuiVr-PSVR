...
namespace VRTK
{
    ...
    public class VRTK_UIPointer : MonoBehaviour
    {
        ...
        public static List<VRTK_UIPointer> Pointers;
        ...
        */
        ...
        {
            UIPointerEventArgs e = new UIPointerEventArgs();
...
#pragma warning restore 0618
            ...
            rayPos = currentRaycastResult.worldPosition;
            return e;
        }
        ...
        protected virtual void OnEnable()
        {
            ...
            if (controller != null)
            {
                ...
                controller.TriggerPressed += DoActivationButtonPressed;
                controller.TriggerReleased += DoActivationButtonReleased;
        ...
        protected virtual void LateUpdate()
        {
            if (controller != null)
            {
                if (pointerEventData == null)
                    ConfigureEventSystem();
                if(pointerEventData != null)
                    pointerEventData.pointerId = pointerID;//(int)VRTK_ControllerReference.GetRealIndex(controllerReference);
            }
            if (lr != null)
            {
                if (On)
                {
                    ...
                    if (rayPos != Vector3.zero)
                        dist = Vector3.Distance(originalPointerOriginTransform.position, rayPos);
                    lr.SetPositions(new Vector3[] { originalPointerOriginTransform.position, originalPointerOriginTransform.position + originalPointerOriginTransform.forward * dist });
                }
                else
