...
public class TeleportingCreature : GroundCreature {
    ...
    public override void OnHit(ArrowCollision e)
    {
        base.OnHit(e);
        if(!health.isDead() && state == EnemyState.walking && e.isMine && canTP)
        {
            TeleportForward();
        }       
    }
    ...
    [PunRPC]
    void TPSpawn(Vector3 Spos, Vector3 Epos)
    {
        Instantiate(TeleportParticles, Spos, Quaternion.identity);
        Instantiate(TeleportParticles, Epos, Quaternion.identity);
    }
}
