...
public class RFX4_UVAnimation : MonoBehaviour
{
    ...
    private int[] _textureNameIds; // Cached texture id
    ...
    {
        _textureNameIds = new int[TextureNames.Length];
        for(int i=0;i<TextureNames.Length;++i)
        {
            _textureNameIds[i] = Shader.PropertyToID(TextureNames[i].ToString());
        }
        ...
        {
            foreach (var id in _textureNameIds) {
                instanceMaterial.SetTextureScale(id, size);
                instanceMaterial.SetTextureOffset(id, offset);
            }
        }
    }
