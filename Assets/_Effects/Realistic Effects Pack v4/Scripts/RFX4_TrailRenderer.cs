...
public class RFX4_TrailRenderer : MonoBehaviour
{
    ...
    private void UpdatePositionsCount()
    {
        ...
        {
            if (Vector3.Distance(t.position, positions[i]) > MaxVertexDistance)
                x = i;
        }
        if (x >= 1)
            RemovePositionsBeforeIndex(x - 1);
    }
    ...
    void AddInterpolatedPositions(Vector3 start, Vector3 end)
    {
        ...
        var previousTime = currentTimes.Count > 0 ? currentTimes[currentTimes.Count-1] : 0f;
...
#region Bezier
    ...
    private List<Vector3> FindDrawingPoints(int curveIndex)
    {
        ...
        pointList.Add(right);
        curCount = 0;
        FindDrawingPoints(curveIndex, 0, 1, pointList, 1, 0);
    ...
    private int FindDrawingPoints(int curveIndex, float t0, float t1,
        List<Vector3> pointList, int insertionIndex, int curCount)
    {
        ...
        if (Vector3.Dot(leftDirection, rightDirection) > DivisionThreshold || Mathf.Abs(tMid - 0.5f) < 0.0001f)
        {
            ...
            {
                curCount++;
                pointsAddedCount += FindDrawingPoints(curveIndex, t0, tMid, pointList, insertionIndex, curCount);
                pointList.Insert(insertionIndex + pointsAddedCount, mid);
                pointsAddedCount++;
                curCount++;
                pointsAddedCount += FindDrawingPoints(curveIndex, tMid, t1, pointList, insertionIndex + pointsAddedCount, curCount);
            }
