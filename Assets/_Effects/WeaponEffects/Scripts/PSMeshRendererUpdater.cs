...
public class PSMeshRendererUpdater : MonoBehaviour
{
    ...
    {
        meshMatEff.mat = new Material(meshMatEff.mat);
        meshMatEff.mat.name = meshMatEff.mat.name + " (Instance)";
        if (isFirst) {
            return new[] { meshMatEff.mat };
        }
        var matsCopyNew = new Material[sharedMaterials.Length + 1];
        sharedMaterials.CopyTo(matsCopyNew, 0);

        matsCopyNew[matsCopyNew.Length - 1] = meshMatEff.mat;
        return matsCopyNew;
    }
}