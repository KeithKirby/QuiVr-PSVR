...
Category {
	...
	SubShader {
		Pass {
			...
			#pragma multi_compile __ _MobileDepth_ON
			#pragma multi_compile ___ _MobileBloom_ON
#pragma target 3.0
...
#endif
#ifdef FresnelFade_ON
				o.fresnel = abs(dot((float3) normalize(v.normal),(float3) normalize(ObjSpaceViewDir(v.vertex))));
			...
			{
			//float d =1- Linear01Depth (UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(i.projPos))));
			//d*=d * d*d *d*d*d*d;
			//return float4(d,d,d,1);
					...
					i.color.a *= fade;
				#else 
				#ifdef _MobileDepth_ON
					float z = tex2Dproj(_CameraDepthTextureSelf, UNITY_PROJ_COORD(i.projPos));
					float sceneZ = LinearEyeDepth (UNITY_SAMPLE_DEPTH(z));
					float partZ = i.projPos.z;
					float fade = saturate (_InvFade * (sceneZ-partZ));
			...
			#endif
			#ifdef _MobileBloom_ON
				return half4((res.rgb - 0.5)/4, res.a);
			#endif
				return res;
			}
			ENDCG 
