...
public class RFX1_ShieldCollisionTrigger : MonoBehaviour
{
    ...
    public void OnArrow(ArrowCollision col)
    {
        foreach (var effect in EffectOnCollision)
        {
            var instance = Instantiate(effect, col.impactPos + col.ImpactNormal * CollisionOffset, new Quaternion()) as GameObject;
            instance.transform.LookAt(col.impactPos + col.ImpactNormal + col.ImpactNormal * CollisionOffset);
            if (!CollisionEffectInWorldSpace) instance.transform.parent = transform;
            Destroy(instance, DestroyTimeDelay);
        }
    }
