...
public class BookControl : MonoBehaviour
{
    ...
    private void SetBookArt()
    {
        ...
        if (MainCover != null && UseTexture)
        {
            MainCover.mainTexture = Book.GetCoverTexture();
        }
        if (bookInSideFrontPage != null && UseTexture)
        {
            bookInSideFrontPage.mainTexture = Book.GetPage(pageIndex);
        }
        pageIndex++;

        if (bookInSideBackPage != null && UseTexture)
        {
    ...
    public void UpdatePageTextures()
    {
        if (!UseTexture)
            return;
        bookInSideFrontPage.mainTexture = Book.GetPage(pageIndex-1);
        bookInSideBackPage.mainTexture = Book.GetPage(pageIndex);
    }
    ...
    {
        if (pageFrontMaterial != null && UseTexture)
        {
    ...
    {
        if (pageBackMaterial != null && UseTexture)
        {
