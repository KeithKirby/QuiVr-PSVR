...
public class OnScreenLog : MonoBehaviour
{
    ...
    void OnGUI()
    {
        lock (syncObject)
        {
            ...
            float screenLogHeight = (Screen.height * 0.95f) - (style.lineHeight + 4); // Space on the screen to show the log
            ...
            foreach (Message msg in log)
            {
                if (y >= 0 && y < (screenLogHeight))
                {
                    ...
                    GUI.color = msg.color;
                    GUI.Label(new Rect(Screen.width * 0.26f, y, Screen.width * 0.74f, style.lineHeight + 4), logText, style);
                }
