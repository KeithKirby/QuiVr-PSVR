	...
	public class CSSceneTools
	{
		...
		public static bool SaveCurrentModifiedScenes(bool skipUntitled)
		{
			...
			for (var i = 0; i < EditorSceneManager.loadedSceneCount; i++)
			{
				...
				if (scene.isDirty)
				{
					var untitled = string.IsNullOrEmpty(scene.name) && string.IsNullOrEmpty(scene.path);

					if (untitled)
					{
						if (skipUntitled) continue;
						scenesPaths += "\nUntitled";
					}
					else
					{
			...
			if (EditorUtility.DisplayDialog("Scene(s) Have Been Modified",
				"All scenes should be saved to continue.\n\nWould you like to save the changes you made in the scenes:" + scenesPaths, "Save", "Cancel"))
			{
				return EditorSceneManager.SaveScenes(scenesToSave.ToArray());
			}
