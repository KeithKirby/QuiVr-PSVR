...
namespace SWS
{
    ...
    public class navMove : MonoBehaviour
    {
        ...
        private IEnumerator NextWaypoint()
        {
            ...
            if(XYOffset != 0 && currentPoint > 0 && currentPoint < waypoints.Length-1)
            {
                Vector3 dirToNext = (waypoints[currentPoint].position - waypoints[currentPoint+1].position);
                offset = Vector3.Cross(dirToNext.normalized, Vector3.up).normalized * XYOffset;
            }
            ...
            if (XYOffset != 0)
            {
                ...
                NavMesh.CalculatePath(transform.position, next.position + offset, areaMask, path);
                if(path.status == NavMeshPathStatus.PathComplete)
                    agent.SetDestination(next.position + offset);
                else
                    agent.SetDestination(next.position);
            }
            else             //set destination to the next waypoint
                agent.SetDestination(next.position);
