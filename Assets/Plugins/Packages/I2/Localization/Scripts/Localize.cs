		...
		public T FindTranslatedObject<T>( string value ) where T : UnityEngine.Object
		{
			...
			if (TranslatedObjects!=null)
            {
			    for (int i=0, imax=TranslatedObjects.Length; i<imax; ++i)
                    if (TranslatedObjects[i] as T != null && value.EndsWith(TranslatedObjects[i].name, System.StringComparison.OrdinalIgnoreCase))
                    {
                        // Check if the value is just the name or has a path
                        if (string.Compare(value, TranslatedObjects[i].name, true)==0)
