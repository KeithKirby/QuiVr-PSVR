...
namespace I2.Loc
{
	...
	public partial class LocalizationEditor
	{
        ...
        static void ParseTermsInScripts() 
		{
			...
			string mScriptLocalization = @"ScriptLocalization\.Get\s?\(\s?\""(.*?)\""\s?\)";
			string mLocalizationManager = @"LocalizationManager\.GetTermTranslation\s?\(\s?\""(.*?)\""\s?\)";
            Regex regex = new Regex(mScriptLocalization + "|" + mLocalizationManager, RegexOptions.Multiline);
