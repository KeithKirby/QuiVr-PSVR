...
namespace I2.Loc
{
	...
	public class UpgradeManager
	{
		...
		{
			I2.I2Analytics.PluginsVersion["I2 Localization"] = LocalizationManager.GetVersion();
            I2.I2Analytics.SendAnalytics("I2 Localization", LocalizationManager.GetVersion()); // Tracks Unity version usage to know when is safe to discontinue support to old unity versions (this allows me to focus more where is more useful to you)
		...
		static void EnablePluginsOnPlatform( BuildTargetGroup Platform )
		{
			...
			HasChanged |= UpdateSettings("TextMeshPro", "TMPro.TMP_FontAsset", "", ref symbols, false );
		...
		static bool UpdateSettings( string mPlugin, string mType, string AssemblyType, ref List<string> symbols, bool removeIfNotFound=true)
		{
			try
			{
				...
				if (hasPluginClass != hasPluginDef)
				{
					...
					if (removeIfNotFound) symbols.Remove(mPlugin);
					return true;
				}
			}
		...
		static void CreateScriptLocalization()
		{
            ...
            string ScriptsFolder = GetI2Path();
