	...
	public partial class LanguageSource : MonoBehaviour
	{
		...
		public int GetLanguageIndex( string language, bool AllowDiscartingRegion = true, bool SkipDisabled = true)
		{
			...
			for (int i=0, imax=mLanguages.Count; i<imax; ++i)
				if ((!SkipDisabled || mLanguages[i].IsEnabled()) && string.Compare(mLanguages[i].Name, language, System.StringComparison.OrdinalIgnoreCase)==0)
					return i;
		...
		public int GetLanguageIndexFromCode( string Code, bool exactMatch=true )
		{
			...
			if (!exactMatch)
			{
				...
				for (int i=0, imax=mLanguages.Count; i<imax; ++i)
					if (string.Compare( mLanguages[i].Code, 0, Code, 0, 2, System.StringComparison.OrdinalIgnoreCase )==0)
						return i;
			}
		...
		public List<string> GetLanguagesCode(bool allowRegions = true, bool skipDisabled = true)
		{
			...
			for (int j = 0, jmax = mLanguages.Count; j < jmax; ++j)
			{
				...
				var code = allowRegions ? mLanguages[j].Code : mLanguages[j].Code.Substring(0, 2);
		...
		{
			int Index = GetLanguageIndex(LocalizationManager.CurrentLanguage, SkipDisabled:false);
		...
		public List<string> GetTermsList ( string Category = null )
		{
			...
			else
			{
				...
				{
					var term = mTerms[i];
					if (LanguageSource.GetCategoryFromFullTerm( term.Term ) == Category)
						terms.Add( term.Term );
				}
				return terms;
			}
